...
// As a function of the flight distance
  ...
  virtual Hep3Vector delDirect( double ) const = 0; // 2nd deriv. WRT pathlen
  virtual double     curvature( double ) const = 0; // = |delDirect|
// For more efficient calling:
  virtual void  getInfo(double fltLen, HepPoint& pos, Hep3Vector& direction) const = 0;
  virtual void  getInfo(double fltLen, HepPoint& pos, Hep3Vector& direction,
                                       Hep3Vector& delDirect) const = 0;
...
inline double Trajectory::lowRange() const {return flightrange[0];}
inline double Trajectory::hiRange() const {return flightrange[1];}
inline double Trajectory::range() const  { return hiRange()-lowRange(); }
#endif

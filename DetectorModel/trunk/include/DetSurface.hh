...
//  surface can be calculated as surfpoint = testpoint + dist*norm.
  virtual 
  double normTo( const HepPoint& testpoint ,Hep3Vector& norm) const = 0;
//
// same, including computation of the associated surface point
  double normalTo( const HepPoint&,Hep3Vector&,SurfacePoint&) const;
//
...
//
  virtual 
  intertype distTo(const HepPoint& testpoint ,const Hep3Vector& dir,
		   double& dist,
		   intermode mode=closest) const = 0;
// same, returning surface point.  This onl
  intertype distanceTo( const HepPoint&,const Hep3Vector&, double&,
...
//
  ...
  Hep3Vector basis(int ax=I_z) const;
  Hep3Vector axis() const;
  HepPoint   gotoLocal ( const HepPoint& )   const;
  HepPoint   gotoGlobal( const HepPoint& )   const;
  Hep3Vector gotoLocal ( const Hep3Vector& ) const;
  Hep3Vector gotoGlobal( const Hep3Vector& ) const;
//
...
//  normal to the surface at a surface point for the given surface coordinates
  virtual Hep3Vector normal(const SurfacePoint&) const = 0;
//  Surface coordinate directions at a surface point
  virtual Hep3Vector surfaceDirection(const SurfacePoint&,int) const = 0;

...
HepTransformation::HepTransformation(const HepRotation & m,
	                             const HepTranslation & p) {
  trans_r = m;
  trans_t = p;
}
...
HepTransformation&
HepTransformation::operator=(const HepTransformation & q) {
...
HepTransformation::HepTransformation( const Hep3Vector& o, const Hep3Vector & z)
  : trans_t(o)
{
  ...
  double cp = cos(z.phi());
  double sp = sin(z.phi());
  trans_r = HepRotation( HepRep3x3( -ct*cp ,  sp, st*cp,
		      		    -ct*sp , -cp, st*sp,
				    st    ,  0., ct     ));
}

HepTransformation::HepTransformation( const Hep3Vector& o,
				      const Hep3Vector& z,
				      const Hep3Vector& x )
  : trans_t(o)
{
  ...
  trans_r = HepRotation( HepRep3x3( i.x(), j.x(), k.x(),
				    i.y(), j.y(), k.y(),
				    i.z(), j.z(), k.z() ));
}

HepTransformation::HepTransformation( const Hep3Vector& o,
				      const EulerAngles& eang )
  : trans_t(o), trans_r()
{
...
HepTransformation&
HepTransformation::operator *=(const HepTransformation& trans2)
{
//
...
HepTransformation&
HepTransformation::transform(const HepTransformation& trans2)
{
//
...
HepTransformation
HepTransformation::inverse(){
  return HepTransformation(trans_r.inverse(),
			   trans_r.inverse()*
			   trans_t.inverse().trans_vec());
}

HepRotation
HepTransformation::rot_mat() const
{
  return trans_r;
}

HepTranslation
HepTransformation::tra_vec() const
{
  return trans_t;
}

HepPoint
HepTransformation::origin( )  const
{
...
Hep3Vector
HepTransformation::unit( int axis ) const
{
  double i(0.), j(0.), k(0.);
  switch (axis) {
  case 0 :
...
HepPoint
HepTransformation::transFrom( const HepPoint& p2  ) const
{
...
HepPoint
HepTransformation::transTo  ( const HepPoint& p1  ) const
{
...
//  given its coordinates in the second system
Hep3Vector
HepTransformation::transFrom( const Hep3Vector& v2 ) const
{
  return trans_r*v2;
}
...
//  given its coordinates in the first system
Hep3Vector
HepTransformation::transTo  ( const Hep3Vector&  v1 ) const
{
  return trans_r.inverse()*v1;
}

Hep3Vector &
HepTransformation::transform( Hep3Vector & v ) const {
  return v = rot_mat() * v;
}

...
class BbrPointErr : public HepPoint {
   public:
    ...
    BbrPointErr(const HepPoint& p) : HepPoint(p), _covMatrix(3) {}
    BbrPointErr(const HepPoint& p, const BbrError& covMat) : HepPoint(p), _covMatrix(3) {
        _covMatrix = covMat;
    }
    ...
    BbrPointErr& transform(const HepGeom::Translate3D& trans) {
        HepPoint::transform(trans);
        return *this;
    }

    BbrPointErr& transform(const HepGeom::Rotate3D& rot) {
        HepPoint::transform(rot);
        _covMatrix = _covMatrix.similarity(rot.getRotation());
        return *this;
    }

    BbrPointErr& transform(const HepGeom::Transform3D& transf) {
        HepPoint::transform(transf);
        _covMatrix = _covMatrix.similarity(transf.getRotation());
        return *this;
    }

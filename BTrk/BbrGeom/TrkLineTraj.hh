...
//
  TrkLineTraj( const HepPoint& point, const CLHEP::Hep3Vector& direction, double length);
  TrkLineTraj(const HepPoint& point, const CLHEP::Hep3Vector& direction, double lorange, double hirange);
...
// needed implementations for intersection with a Surface
  ...
  CLHEP::Hep3Vector direction( double ) const;
  double curvature( double f = 0. ) const;
  CLHEP::Hep3Vector delDirect( double ) const;
  void  getInfo(double fltLen, HepPoint& , CLHEP::Hep3Vector& direction) const;
  void  getInfo(double fltLen, HepPoint& , CLHEP::Hep3Vector& direction, 
			CLHEP::Hep3Vector& delDirect) const;
...
private:
  ...
  CLHEP::Hep3Vector _direction; // direction (unit) vector
};
#endif

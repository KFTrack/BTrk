...
TrkDifPieceTraj::TrkDifPieceTraj(const std::vector<TrkSimpTraj*>& trajs) {
    ...
    for (std::vector<TrkSimpTraj*>::const_iterator itraj = strajs.begin(); itraj != strajs.end();
         itraj++) {
        ...
        if (prevtraj != 0) {
            ...
            if (add.failure()) {
                ErrMsg(warning) << "construction from vector of trajs failed" << add << endmsg;
                delete newtraj;
                break;
            }
        } else {
...
void TrkDifPieceTraj::getDFInfo(double flightdist,
                                DifPoint& pos,
                                DifVector& direction,
...
void TrkDifPieceTraj::getDFInfo2(double flightdist, DifPoint& pos, DifVector& direction) const {
    //
    //  First, find the right trajectory piece, then give the local position
    //
    double localflight(0.0);
...
//
const TrkErrCode& TrkDifPieceTraj::append(double glen, TrkSimpTraj* nexttraj, double& gap) {
    ...
    if (glen >= lowRange()) {
        ...
        if (nremoved > 0)
            ErrMsg(warning) << "append removed " << nremoved << " old trajectories" << endmsg;
...
const TrkErrCode& TrkDifPieceTraj::prepend(double glen, TrkSimpTraj* nexttraj, double& gap) {
    static TrkErrCode retval;
    retval = TrkErrCode(TrkErrCode::succeed);
    // simple range checks
    if (glen <= hiRange()) {
        ...
        if (nremoved > 0)
            ErrMsg(warning) << " prepend removed " << nremoved << " old trajectories" << endmsg;
        //  Compute POCA for this trajectory to the point at the end of the existing trajectory
        HepPoint endpoint = position(glen);
...
const TrkErrCode& TrkDifPieceTraj::append(double glen, const TrkSimpTraj& nexttraj, double& gap) {
    return append(glen, nexttraj.clone(), gap);
}

const TrkErrCode& TrkDifPieceTraj::prepend(double glen, const TrkSimpTraj& nexttraj, double& gap) {
    return prepend(glen, nexttraj.clone(), gap);
}

// append an entire TrkDifTraj
const TrkErrCode& TrkDifPieceTraj::append(double glen, const TrkDifPieceTraj& other, double& gap) {
    static TrkErrCode retval;
    retval = TrkErrCode(TrkErrCode::succeed);
...
const TrkErrCode& TrkDifPieceTraj::prepend(double glen, const TrkDifPieceTraj& other, double& gap) {
    static TrkErrCode retval;
    retval = TrkErrCode(TrkErrCode::succeed);
    // find POCA between the trajectories.
    double mylen = glen;
...
int TrkDifPieceTraj::trajIndex(const double& flightdist, double& localflight) const {
    ...
    if (_localtraj.size() > 1) {
        if (validFlightDistance(flightdist)) {
            ...
            if (_lastIndex >= 0 && _lastIndex < static_cast<int>(_localtraj.size()) - 1 &&
                flightdist > _globalrange[_lastIndex] &&
                flightdist < _globalrange[_lastIndex + 1]) {
                index = _lastIndex;
            } else {
                //
                //  simple binary search algorithm
                //
                index = std::upper_bound(_globalrange.begin(), _globalrange.end(), flightdist) -
                        _globalrange.begin() - 1;
                if (index >= static_cast<int>(_localtraj.size()))
                    index = static_cast<int>(_localtraj.size()) - 1;
            }
        } else {
            //
...
//  Print functions
//
void TrkDifPieceTraj::print(ostream& os) const {
    os << "TrkDifPieceTraj has " << _localtraj.size() << " pieces "
       << ", total flight range of " << hiRange();
}

void TrkDifPieceTraj::printAll(ostream& os) const {
    os << "TrkDifPieceTraj has " << _localtraj.size() << " pieces "
       << ", total flight range from " << lowRange() << " to " << hiRange() << endl;
    for (int ipiece = 0; ipiece < static_cast<int>(_localtraj.size()); ipiece++) {
        ...
        os << "Piece " << ipiece << " has global range from " << _globalrange[ipiece] << " to "
           << _globalrange[ipiece + 1] << " and local range from " << plow << " to " << phi << endl;
        HepPoint start = tpiece->position(plow);
        HepPoint end = tpiece->position(phi);
        const HepPoint& refp = tpiece->referencePoint();
        os << "Piece " << ipiece << " starts at point " << start.x() << "," << start.y() << ","
           << start.z() << " and ends at point " << end.x() << "," << end.y() << "," << end.z()
           << " and references point " << refp.x() << "," << refp.y() << "," << refp.z() << endl;
    }
}
//
//  Range functions
...
void TrkDifPieceTraj::setFlightRange(double newrange[2]) {
    ...
    //
    if (newrange[1] > newrange[0] && newrange[0] < flightrange[1] && newrange[1] > flightrange[0]) {
...
//  KalDeriv functions
//
HepMatrix TrkDifPieceTraj::derivDeflect(double flightdist, deflectDirection idir) const {
    double localflight(0.0);
    const TrkSimpTraj* loctraj = localTrajectory(flightdist, localflight);
    return loctraj->derivDeflect(localflight, idir);
}

HepMatrix TrkDifPieceTraj::derivDisplace(double flightdist, deflectDirection idir) const {
    double localflight(0.0);
    const TrkSimpTraj* loctraj = localTrajectory(flightdist, localflight);
    return loctraj->derivDisplace(localflight, idir);
}

HepMatrix TrkDifPieceTraj::derivPFract(double flightdist) const {
    double localflight(0.0);
    const TrkSimpTraj* loctraj = localTrajectory(flightdist, localflight);
    return loctraj->derivPFract(localflight);
}

const TrkErrCode& TrkDifPieceTraj::append(TrkSimpTraj* newtraj, double& gap) {
    static TrkErrCode retval;
    retval = TrkErrCode(TrkErrCode::succeed);
        ...
        if (index == static_cast<int>(_localtraj.size()) - 1) {
            if (endpoca.flt1() - hiRange() > _SMALLGAP) {
                ...
                if (midpoca.status().success() && midpoca.flt1() < newtraj->lowRange()) {
                    ...
                    // append the new global range
                    _globalrange.back() =
                        midflt;  // override old global fltlen, now goes to midpoint
                    _globalrange.push_back(_globalrange.back() + midrange);  // gaptraj global
                                                                             // range, from midpoint
                                                                             // poca to lowRange()
                                                                             // of new traj
                    _globalrange.push_back(_globalrange.back() +
                                           newtraj->range());  // add range of the new traj
                ...
                range[1] = local;
                _localtraj.back()->setFlightRange(
                    range);  // override local traj range to end at POCA of new piece
                _localtraj.push_back(newtraj);
                _globalrange.back() =
                    endpoca.flt1();  // override old global fltlen, now goes to start of new piece
...
const TrkErrCode& TrkDifPieceTraj::prepend(TrkSimpTraj* newtraj, double& gap) {
    static TrkErrCode retval;
    retval = TrkErrCode(TrkErrCode::succeed);
    // find out where this piece is relative to the existing pieces.
...
bool TrkDifPieceTraj::locallyValid(double glen, double tol) const {
    double localflight(0.0);
    const TrkSimpTraj* loctraj = localTrajectory(glen, localflight);
    return loctraj != 0 && localflight - tol <= loctraj->hiRange() &&
...
bool TrkDifPieceTraj::operator==(const TrkDifPieceTraj& other) const {
    bool retval(true);
    if (retval)
        retval = _globalrange == other._globalrange;
    if (retval)
        retval = _localtraj.size() == other._localtraj.size();

...
public:
  ...
  (double s,const CLHEP::HepVector& d); 
  inline
...
public:
  ...
  inline void setDerivatives(const CLHEP::HepVector& d)   { copyDerivs(d); }
  ...
  CLHEP::HepVector derivatives()const;
  ...
  double error(const CLHEP::HepSymMatrix& e)const; // error from specified error
  ...
  double correlation(const DifNumber& b,const CLHEP::HepSymMatrix& e) const;	// correlation for specified error
  double correlation(const DifNumber& b) const; // correlation from default matrix

  //sum into functions
  void sumMatrix(CLHEP::HepMatrix& m)const;

  //fetch functions
  void fetchNumber(double& n)const  {n=number();}
  void fetchDerivatives(CLHEP::HepVector& v)const;
  ...
  friend double correlation(const DifNumber& a,const DifNumber& b,const CLHEP::HepSymMatrix& e);	// correlation for specified error
...
private:
  ...
  inline DifNumber& copyDerivs(const CLHEP::HepVector& v); // copy derivatives v
...
#include "BTrk/difAlgebra/DifNumber.icc"

...
public:
  ...
  KalWeight(const CLHEP::HepVector&, const CLHEP::HepSymMatrix&);
  KalWeight(const KalParams&);
  KalWeight(const KalWeight&);
  ~KalWeight(){;}
  unsigned nPar() const { return _wvector.num_row(); }
  const CLHEP::HepVector& weightVector() const { return _wvector; }
  const CLHEP::HepSymMatrix& weightMatrix() const { return _wcov; }
...
private:
  CLHEP::HepVector _wvector;
  CLHEP::HepSymMatrix _wcov;
  int _status;
};
#endif

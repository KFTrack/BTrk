...
BFMap::BFMap( const std::string& key, bool warnIfOutside):
  _key(key),
...
BFMap::BFMap(const std::string& filename, 
...
bool BFMap::isValid(Hep3Vector const& point) const{
...
void BFMap::getNeighbors( int ix, int iy, int iz, 
			  Container3D<Hep3Vector>& neighborsBF) const {
  for (int i = 0; i != 3; ++i){
...
Hep3Vector BFMap::interpolate(Container3D<Hep3Vector> const vec,
...
Hep3Vector BFMap::getBField(Hep3Vector const& testpoint ) const{
  ...
  //cout << "Point: " << point+Hep3Vector(0.,-7350.,-4000.) << endl;
